...
class ApimartVideo(BaseTool):
    ...
    def _build_payload(self, inputs: dict[str, Any], model: str) -> dict[str, Any]:
        ...
        if family.startswith("kling"):
            payload["mode"] = _KLING_MODES[resolution]
        else:
        ...
        if audio_key and inputs.get("generate_audio") is not None and "video_list" not in payload:
            payload[audio_key] = bool(inputs["generate_audio"])
